...
func NewFunctionTemplateXXXXXXXXXXXX(iso *Isolate, callback FunctionCallback) (*FunctionTemplate, error) {
	if iso == nil {
...
func goFunctionCallback(ctxref int, cbref int, args *C.ValuePtr, argsCount int) C.ValuePtr {
	...
	argv := (*[1 << 30]C.ValuePtr)(unsafe.Pointer(args))[1 : argsCount+1 : argsCount+1]
	for i, v := range argv {
